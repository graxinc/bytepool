...
import (
	...
	bytepool "github.com/graxinc/bytepool/v2"
)
		...
		for range 100 { // can have a buf dropped sometimes
			...
			want := bytepool.BucketPoolStats{
				Buckets: []bytepool.BucketStats{
					...
					{Size: 4, Puts: 2, Hits: 1, Misses: 1},
					{Size: 8, Puts: 4, Hits: 3, Misses: 1},
					{Size: 9, Puts: 1, Misses: 1},
				},
				MinSize:     2,
				MaxSize:     9,
				Sizes:       4,
...
func TestBucket_getChoice(t *testing.T) {
	...
	fill := func(b *bytepool.Bytes, n int) {
	...
	cases := []struct {
		...
		{
			fills:     []int{1, 2, 3, 4, 5, 6, 7, 8, 8, 7, 6, 5, 4, 3, 2},
			chooseInc: 3,
			want: bytepool.BucketPoolStats{
				Buckets: []bytepool.BucketStats{
					{Size: 2, Puts: 1, Hits: 2, Misses: 2},
					{Size: 4, Puts: 1, Misses: 3},
					{Size: 8, Puts: 2, Hits: 8},
				},
				MinSize:     2,
				MaxSize:     8,
				Sizes:       3,
	...
	for _, c := range cases {
		t.Run("", func(t *testing.T) {
			...
			var lastDiff string
			for range 100 { // can have a buf dropped sometimes
				pool := bytepool.NewBucketFull(sizes, bytepool.BucketPoolOptions{ChooseInc: c.chooseInc})
				...
				lastDiff = cmp.Diff(c.want, pool.Stats())
				if lastDiff == "" {
					return
				}
			}
...
func TestBucket_getChoice_concurrent(t *testing.T) {
	...
	normInt := func(rando *rand.Rand, n int, center float64) int {
		...
		vf := f*(float64(n)/6) + float64(n)*center
	...
	run := func(t *testing.T, center float64, wantDefMin, wantDefMax int) {
		...
		pool := bytepool.NewBucketFull(sizes, bytepool.BucketPoolOptions{ChooseInc: 200})
	...
	t.Run("center=0.75", func(t *testing.T) { run(t, 0.75, 650, 780) })
}
